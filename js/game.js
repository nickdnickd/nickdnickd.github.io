(() => {
  ...
  const TYPES = [
    ...
    {name:'DEBRIS APPROACH', short:'OBJ', need:'pulse', issue:'COLLISION VECTOR CONFIRMED', hint:'A kinetic pulse will deflect the object.'}
  ];
  let state, stars=[];
  ...
  function init(seed=missionSeed()){
    ...
    const picked=[...TYPES].sort(()=>r()-.5).slice(0,4);
    state={seed,turn:1,maxTurns:18,energy:6,health:100,angle:-Math.PI/2,sites:picked.map((t,i)=>({...t,angle:i*Math.PI/2+(r()-.5)*.35,known:false,resolved:false,deadline:7+i*3+Math.floor(r()*2)})),over:false};
  ...
  function advance(){
    ...
    state.sites.forEach(s=>{if(!s.resolved&&state.turn===s.deadline){state.health-=18;log('IMPACT',`${s.known?s.name:'An unidentified site'} failed. Colony integrity −18%.`);s.deadline+=5}});
  ...
  function renderUI(){
    ...
    const site=currentSite();$('#contact').textContent=site?`SECTOR ${state.sites.indexOf(site)+1}`:'DEEP SPACE';$('#targetIndex').textContent=site?`S—${state.sites.indexOf(site)+1}`:'S—';$('#targetName').textContent=site?(site.known?site.name:'UNRESOLVED SIGNAL'):'NO CONTACT';$('#targetDetail').textContent=site?(site.resolved?'SITE STABLE':site.known?site.hint:'Deep scan required'):'Awaiting orbital alignment';
    $('#siteList').innerHTML=state.sites.map((s,i)=>`<div class="site ${s.known?'known':''} ${s.resolved?'resolved':''}"><i></i><span>S${i+1} · ${s.known?s.name:'UNRESOLVED SIGNAL'}</span><em>${s.resolved?'STABLE':s.known?'AT RISK':''}</em></div>`).join('');
  ...
  function draw(){
    ...
    state.sites.forEach((s,i)=>{const x=cx+Math.cos(s.angle)*R*.78,y=cy+Math.sin(s.angle)*R*.78;ctx.fillStyle=s.resolved?'#79c3a3':s.known?'#e2aa6e':'#789087';ctx.beginPath();ctx.arc(x,y,s.known?4:2.5,0,Math.PI*2);ctx.fill();if(s.known){ctx.strokeStyle='rgba(225,169,107,.35)';ctx.beginPath();ctx.arc(x,y,9,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#cddbd5';ctx.font='8px DM Mono';ctx.fillText(`S${i+1}`,x+12,y+3)}});
